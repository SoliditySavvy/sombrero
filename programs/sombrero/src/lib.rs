...
pub mod processor;
pub use processor::*;
pub mod verifying_key_sombrero;
pub use verifying_key_sombrero::*;
...
pub mod sombrero {
    ...
    /// It creates and initializes a verifier state account to save state of a verification during{ VERIFYINGKEY_SOMBRERO.nr_pubinputs }
    /// computation verifying the zero-knowledge proof (ZKP). Additionally, it stores other data
    /// such as leaves, amounts, recipients, nullifiers, etc. to execute the protocol logic
    /// in the last transaction after successful ZKP verification. light_verifier_sdk::light_instruction::LightInstruction2
    pub fn light_instruction_first<'a, 'b, 'c, 'info>(
        ctx: Context<'a, 'b, 'c, 'info, LightInstructionFirst<'info, { VERIFYINGKEY_SOMBRERO.nr_pubinputs }>>,
        inputs: Vec<u8>,
    ) -> Result<()> {
        ...
        let mut checked_inputs: [[u8; 32]; VERIFYINGKEY_SOMBRERO.nr_pubinputs] = [[0u8; 32]; VERIFYINGKEY_SOMBRERO.nr_pubinputs];
        checked_inputs[0] = program_id_hash;
        checked_inputs[1] = inputs_des.transaction_hash;

        process_psp_instruction_first::<{ VERIFYINGKEY_SOMBRERO.nr_pubinputs }, 17>(
            ctx,
            &proof,
            &public_amount,
    ...
    pub fn light_instruction_second<'a, 'b, 'c, 'info>(
        ctx: Context<'a, 'b, 'c, 'info, LightInstructionSecond<'info, { VERIFYINGKEY_SOMBRERO.nr_pubinputs }>>,
        inputs: Vec<u8>,
    ) -> Result<()> {
    ...
    pub fn light_instruction_third<'a, 'b, 'c, 'info>(
        ctx: Context<'a, 'b, 'c, 'info, LightInstructionThird<'info, { VERIFYINGKEY_SOMBRERO.nr_pubinputs }>>,
        inputs: Vec<u8>,
    ) -> Result<()> {
        msg!(
            "checked inputs {:?}",
    ...
    pub fn close_verifier_state<'a, 'b, 'c, 'info>(
        _ctx: Context<'a, 'b, 'c, 'info, CloseVerifierState<'info, { VERIFYINGKEY_SOMBRERO.nr_pubinputs }>>,
    ) -> Result<()> {
        Ok(())
    }
}
